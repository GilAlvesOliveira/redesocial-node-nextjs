...
const seguirEndpoint = async (req: NextApiRequest, res: NextApiResponse<RespostaPadraoMsg>) => {// Declaração padrão conforme os outros endpoints
  try {
    if (req.method === "PUT") {                                  // Verifica se o método HTTP é PUT, caso não seja retorna um erro de método inválido
      ...
      const euJaSigoEsseUsuario = await SeguidorModel.find({      // Procura na DB de seguidor se já há uma correspondência de seguidor e seguido conforme os id's
        usuarioId: usuarioLogado._id,
        usuarioSeguidoId: usuarioASerSeguido._id,
      });
      if (euJaSigoEsseUsuario && euJaSigoEsseUsuario.length > 0) {// Caso já seja seguidor faremos o processo de unfollow
        euJaSigoEsseUsuario.forEach(async (e: any) => await SeguidorModel.findByIdAndDelete({ _id: e._id })); // Para cada correspondencia que retornou procura ele no DB e deleta as correspondências uma a uma
...
export default validarTokenJWT(conectarMongoDB(seguirEndpoint)); // Exporta o endpoint de seguir passando pelos middlewares necessários